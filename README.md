...
3. Додано новий ефект «Смак меду» на базі коду від (© Stepko) адаптация © SlingMaster.
4. Додана нова функція глобальна зміна яскравості 
	(щоб зберегти зміни необхідно перемкнути кнопку в програмному забезпеченні).
5. Додана нова функція використання лампи тільки з підключенням до роутера, дуже актуальна з постійним відключенням електроенергії (якщо пароль і ssid лаипа вже запамятала то через деякий час всерівно підключиться до роутера, перехід в режим точки доступу можливий семиразовим кліком по кнопці )
6. Оновлене програмне забезпечення під всі платформи.
